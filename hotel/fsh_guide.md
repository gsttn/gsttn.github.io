...
* スタンバイパス＆DPAの利用者は新エリアに入ったら出ていかないので
* マジックチケット利用者やスプホ利用者も新エリア内に滞留する
* バケパ以外のミラコ利用者はハッピーエントリーを利用して一般より前に入場する
* ディズニーシー全体（新エリア以外）における混雑度の限界は5万人とする
    * 最近のディズニーシーのチケット販売枚数の日次上限を調べることは出来なかった
    * コロナ前の情報と思われる混雑予想系のサイトだと4万～5万人ほどでディズニーシーへの入場制限がかかるとしているケースが多い


### 事前の入場可能者数予想

まずはオープン前に入場可能となっている利用者数の予想ですが、スプホの客室数がファン棟419＋グラン棟56で475室、仮に稼働率100%で1部屋平均2.5名が宿泊すると約1200名、これをチェックイン日とチェックアウト日の両方で入ってくるとするなら2倍の2400名になりますね
仮に1部屋平均2名なら950名の2日分で1900名、逆に1部屋平均3名なら2850名ですので、概ねこの辺の2000～3000名くらいがスプホ関連の入場者数と言えそうですね。
意外と少ない？多い？どうでしょうかね。

ここにスプホ以外のバケパ勢が入場確約となります。
これも参考となる数字は見つけられませんでしたが、ミラコ、ランホ、アンバ、トイホ、セレブレの5ホテルで各100部屋、1部屋平均2名とすると1000名ですね。
バケパは原則としてマジックチケットの設定が1日分しかないので、2倍（チェックイン日とチェックアウト日の2日分）にはしません。
こちらも平均3名なら1500名ですから、1000名～1500名くらいでしょうか。
仮にバケパ設定が各ホテル200部屋ならその2倍、上限は3000名、雑に1000～3000名くらいがバケパ（スプホ以外）枠と推測できなくもないでしょうかね。
...
まず、新エリアの面積ですが[OLCの発表資料](https://www.olc.co.jp/ja/news/news_tdr/20221027_01/main/0/link/20221027_01(2).pdf)によると、『開発面積：約 140,000 ㎡ （うち、テーマパーク・ホテルエリア 約 100,000 ㎡）』とありますね。
...
つまり、単純な面積比で言うと、新エリア＋スプホの面積はディズニーシー（テーマパークエリア）の1/5程度となり、新エリア部分だけならさらに狭いということになります。
（水の部分や植栽で歩けない部分に関してはひとまず考慮外としているのであくまでも参考程度）

この49.0 haに5万人が集中するとコロナ前の入場制限クラスになる、では10 ha（ホテル込み約10万平米）に対してならその1/5の1万人が集中したら同規模の混雑になると予想出来ます。
ただし、この10 haにはスプホの面積が含まれていますので、**新エリアのキャパシティは1万人よりかなり少ない人数であると推定**出来ますね。

スプホ部分の1階がパーク共有となっているためか、残念ながらスプホの占有面積やパーク内だけの面積は不明です。
とは言え、スプホ＋新エリアで1万人で入場制限クラスなので、仮に2割がスプホだった場合は8千人、3割がスプホなら7千人と計算はし易いですね。
...
もし、コロナ前の入場制限日を超えるくらいの混雑をしても構わないと想定して、ガッツリ1万人としてバケパの設定数と利用者数が極めて少なかったと想定すれば7000名分のスタンバイパス＆DPAが発行されることになりますね。
ただしミラコの宿泊者はハッピーエントリーで先行入場可能なので、400室分（全500室のうち100室はバケパと推定した前提）は先に持っていかれますので、仮に1部屋平均2名として800名分は持っていかれます。

残りは6200名分、保安検査のゲートや列の数を正確に把握してはいないのですが（誰か情報お持ちでしょうか？）仮にノースとサウスに15個ずつ配置されているなら30個、つまり入園待ち1列あたり207名までなら購入可能な計算になります。

ただし、これはスタンバイパスとDPAの合計値だけでカウントしたケースなので、実際にはその配分も問題になります。
確かに、DPAの連続購入やスタンバイパスの連続習得は出来ませんが、DPAとスタンバイパスの両方を取得することは可能です。
つまり、**1名が入って即座にDPAとスタンバイパスの両方を確保する**というして、スタンバイパス：DPA＝1:1の比率で配分される前提で考えると、入園まち1列あたり103名までで打ち止めになります。
あるいは既にマジックチケットが大量に出回っているのでDPAが少なめで9割がスタンバイパスに割り振られると仮定するなら、入園まち1列あたり186名までは取れそうです。

**極めて楽観的に予想するなら概ね荷物検査の待ちで先頭から200名ほどに入ればスタンバイパスとDPAのいずれか発行枚数が多い方を確保可能**となりそうですね。
1グループ平均2.5名とするなら、荷物検査の先頭から80番目のグループまで、1グループの占有幅が1mほどなら先頭から80mまでに並んでおきましょう、という感じでしょうか。
参考までにGoogle Mapで計測するとノースもサウスも先頭から最後尾で100m前後（ノースは100m弱、サウスは100m強）のようですね。


#### 悲観的な予想

まずコロナ前の入場制限クラスまで入れない、ホテル専有面積も考慮、という条件で新エリア入場者を5千人までに絞ったとしましょうか。
スプホとバケパの入場者数の予想が3000～6000名と仮定したので、既にオーバーする可能性がありますね（苦笑

仕方ないので少し楽観的にバケパ設定数が多めのパターンを除外して、3000～4500名なので、500～2000名分くらいがスタンバイパス＆DPAに配分される計算でしょうか。

仮に1500名分として、ミラコのハッピーエントリーで800名分が持っていかれるとして、残りは700名分ですか。
待ち行列が30本あるとして、入園待ち1列あたり23名あたりまで、1グループ平均2.5名として9番目のグループに並んでギリギリ取れるか否か、ですね。

この試算ではスタンバイパスとDPAの両方を持っていくケースを除外しているので、恐らく荷物検査の先頭から10m以内に並んでも厳しい状態でしょう。
もちろん、これは入場時点での人数なので代表者1名が並んで、あとから10名とかが押し寄せて合流するようなケースも考慮すると、先頭から10番目以内に並んでも取れるかは**前のグループ次第**となりそうです。


### アトラクション数比

基本は面積比と同様に入場制限人数からの推測にしますが、次はアトラクションの数で比率を出してみます。
...
コロナ前のシー入場者制限を5万人と置いて、その4/30ですから6700人ほどでしょうか。
スプホとバケパの入場者数の予想が3000～6000名と仮定したので、かなりギリギリですね。

仮に事前確約が3000名と置いて、ミラコのハッピーエントリーを800名、2900名分がスタンバイパス＆DPAとなりますか。
これを荷物検査の待ち行列30本とすると、入園待ち1列あたり96名分、1グループ2.5名で並ぶとしたら38番目のグループまででしょうか。
...
コロナ前のシー入場者制限を5万人と置いて、許容される混雑度をその半分としたら3300人ほどでしょうか。
はい、もうバケパとスプホで完全にアウトですし、ミラコのハッピーエントリーで完全にとどめを刺されるレベルです。

むしろ、ミラコのハッピーエントリーでもかなり前の方に並ばないと取れないというレベルになりますね。
数百枚というレベルなのでスタンバイパスとDPAを1:1で発行していたら、ミラコ宿泊者でも50部屋分を切る可能性すらあり得ます。
...
（スプホ<span id="pre_ent_cnt_sph">－－</span>名、バケパ<span id="pre_ent_cnt_bp">－－</span>名）
...
**一般入場スタンバイパス＆DPA取得可能人数 <span id="od_pass_cnt">－－</span>名分**
...
**保安検査待ちの各列先頭から<span id="od_pass_position">－－</span>番目**
...
function calc_all(){
  ...
  v6 = document.getElementById('od_pass_cnt').innerHTML = Math.ceil((v4 * document.getElementById('sp_dpa_rate').value) - v5)
  document.getElementById('od_pass_position').innerHTML = Math.ceil(v6 / document.getElementById('ent_cnt').value / document.getElementById('ent_person_cnt').value)
}
calc_all();
</script>
