...
[トリビアの面白URL](/common/trivia.md#面白URL色々)にも記載しましたが、特定条件にマッチする複数の部屋を検索するURLを生成する機能をお試しで作ってみました。

<script src="/script/keyword.js"></script>

注意すべき点ですが…

* 本質的に読み込み速度の面では1部屋の方が有利なので、11時とか開幕タイミングのワンポイントで勝負する際には不利
    * 逆に言うと全部屋検索よりはいくらか有利になる局面があるかも
* 部屋名や説明欄を検索しているようなので、それらが更新されると意図しない部屋がマッチする可能性がある
* 一応、表示時点で対象部屋の先頭が開くようにしてあるが、『空きありのみ』にすると開かないケースもある
* では何でこんなモノを作ったのか？というと、どうしても宿泊したい特定日にレアリティがあまり高くない複数のターゲットを緩やかにキャンセル拾いをする際には使えなくもないかも知れないという程度
    * ま、単なる好奇心の結果発表であり、自己顕示欲の発露なのかも知れません（苦笑
