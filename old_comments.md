...
* むしろ価格以外のどんな条件があると公式にデラックスということになるのでしょうかね？
    * リゾラ沿い→トイホもある **×**
    * リゾラの内側にある→何気にアンバは内側、ただしランホは外側にある **×**
    * スイートルームがある→ファン棟にない **×**
    * ルームサービスがある→アンバとファン棟にない **×**
    * プールがある→スプホにない **×**
    * FTW（結婚式プラン）がある→ランホとスプホにない **×**
    * 宴会/パーティプランがある→ランホとスプホにない **×**
    * TDRグッズショップがある→トイホにもある、スプホにない **×**
    * パーク激近→地味にアンバは遠い **×**
    * ウェルカムセンターにカウンターがある→トイホもある **×**
    * 上位の部屋専用ラウンジがある→アンバはなくなり、ファン棟もないと言えばない **×**
    * レストランの数→確かにデラックスは3箇所以上、ただファン棟はラリベは使えないので2箇所、セレブレも2箇所なので微妙に **×**
    * シーにハピエンできる→現時点では **◯**  、ただし、[ランホとアンバは09/14まで](https://www.tokyodisneyresort.jp/hotel/topics/info/happy_entry.html)のようなので判定としては **×**
    * エコバッグがある→[客室にご用意するエコバッグについてのページ](https://www.tokyodisneyresort.jp/hotel/topics/info/ecobag.html)に『ラグジュアリータイプおよびデラックスタイプのディズニーホテルでは』と明記されている **◯**
    * エクスクルーシブグッズがある→[宿泊者しか買えないトートバッグのページ](https://www.tokyodisneyresort.jp/hotel/happy/exclusive_tote_2024/)に『※こちらの特典は、デラックスタイプおよびラグジュアリータイプのホテルのみ対象となります。』と明記されている **◯**
    * ランドリーサービスがある→サービスホットラインで対応してもらえる、トイホとセレブレはコインランドリー **◯**
    * 結論：宿泊者専用のトートバッグとエコバッグがあって、洗濯サービスが手厚いのがデラックス……違う…断じて違う…（苦笑
    * ちなみに、このネタの調査で公式サイトを見ていたのですが、トイホとセレブレって公式サイトに館内マップがないのですね…公式サイトに館内マップがあるのがデラックスの証……これも違う…（苦笑
...
* 少し細かく数字を見ていこうかと思います、ターゲットは2026年11月の宿泊料金なのでクリスマス時期のプレミアは載っているかも知れません、いずれにしろグランドシャトーは別格ですが…
    * ざっくりと価格チェック、ひとまずスイートは除外、ま、グランドシャトーは別格ですが…
        * ファンタジーシャトー：ベースとなるベイエリア側とエントランス側で10万オーバーくらい、ローズコートも大差なし、スプリングスサイドの上層とバルコニーで13～14万くらい
        * グランドシャトー：33万～47万くらい
...
* 未だにグランドシャトーとバケパを比較するような話ってあるのでしょうかね？とか思いつつ過去ネタに手を加えて[グランドシャトー宿泊対策](./hotel/fsh_grand_chateau_guide.md#敢えてバケパと比較)のページに追記してみました
    * ま、大半は以前のコメントネタの再掲です、他にも検討要素がありそうなので[ツッコミはマシュマロまで](https://marshmallow-qa.com/ecu5ejrsz5qmtuu)お願いします
* AIさんの進化がスゴイですね、という昨今、2年弱前に[生成AIと遊ぼう](./common/play_with_gpt.md)にてDヲタ用語を説明させて笑わせてくれたAIさんですが、現在のAIさんに再チャレンジしてもらいました
    * 結果は予想通りですね


## 2026-03-02

* おや、ドリーマーズラウンジはアフタヌーンティーセットとパスタセットの2枚看板をやめて、スペシャルセットに集約した感じなのですかね
    * 単純な値上げよりは良いかも知れませんが、ミニカップスープからの3段トレイ＋パスタ＋デザートって、まずまずなボリューム感にはなりますね（若者には不足としても）
    * このおかげで、ハイピのプレミアムスイーツに価格逆転される最悪のケース（？）は回避されましたね、『アンバ？奴は我々、デラックス四天王でも最弱…』
* ハイピのプレミアムスイーツが[スウィーツドリンクとのセット割引](https://www.tokyodisneyresort.jp/hotel/food/1000005292/)を始める、ということになるのですかね、これは
    * [スウィーツドリンク](https://www.tokyodisneyresort.jp/hotel/food/1000005293/) \2,200 → \1,800ですか、ま、そもそもプレミアムスイーツの方がガンガン値上げされていますが…
...
* 別にどちらでも良いことではあるのですが、ハイピでスペドリ以外の隠しメニューなドリンクや年末年始メニューのワインを詳しめに勧める感じになったのは客単価の向上施策の一環なのでしょうかね
    * 思うに、どうでも良いようｎ……ちょっとした限定チャームのようなオマケを付ければ飛ぶように売れるはずなので、そちらの方針を採れば良いのではないかな、と…
* ついにファンタジースプリングスのエリア入場が自由になりつつあるようですね
    * アトラクションのスタンバイパス/DPAと、レストランのモバイルオーダーが必須な状況は変わっていないようですが、閑散期に様子見しながらこの辺も徐々に開放されていくのでしょう
    * さておき、ファンタジースプリングスホテルの方はそろそろ1周年の予約開放が近づいてきていますが、さすがのDホテルヲタ勢でもそこまで話題になるほどではない、という感じなのでしょうかね（苦笑
